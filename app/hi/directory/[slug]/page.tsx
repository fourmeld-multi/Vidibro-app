  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        hi: url,
        bn: localeUrl(slug, "bn"),
      },
    },
    openGraph: { title, description, url, siteName: "Vidibro", locale: "hi_IN" },
  };
}

export default async function HiDirectoryPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <main lang="hi" className="w-full mx-auto max-w-3xl px-5 py-12">
      <JsonLd
        data={[
          {
      ...
      <p className="mt-10 text-sm text-purple-300/60">
        ...
        {" · "}
        <Link href={`/bn/directory/${slug}`} className="underline hover:text-purple-200">
          বাংলা
        </Link>
      </p>
    </main>
