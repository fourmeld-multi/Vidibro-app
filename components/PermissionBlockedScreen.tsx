...
export default function PermissionBlockedScreen({
  ...
  const device = isAudio ? "microphone" : "camera and microphone";
  // Match the icon to what is actually blocked — a camera glyph on the audio
  // screen reads as the wrong permission being at fault.
  const Icon = isAudio ? MicOff : CameraOff;
    ...
    <div className="fixed inset-0 z-[90] flex flex-col items-center justify-center gap-5 bg-[#070414] px-8 text-center">
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-lg font-extrabold text-white">
          {isAudio ? "Audio" : "Video"} chat is blocked
        </h2>
        <p className="max-w-xs text-xs leading-relaxed text-purple-200/70">
          ...
          again from here. Tap the lock or {isAudio ? "microphone" : "camera"} icon in
          the address bar, set the{" "}
